...
namespace NexusBench
{
    ...
    public static class BinderSuite
    {
        ...
        // =========================================================================
        // AOT — NexusGeneratedBinder.g.cs drift guard (conditional: file is gitignored
        // and only exists after the Unity editor generates it)
        // =========================================================================

        private static void RunBinderAotCompatibility()
        {
            // Harness runs from tools/nexus-benchmark (../../Nexus/...) or from the repo
            // root via --project (Nexus/...). Try both; skip silently if neither exists.
            string file = null;
            string[] candidates =
            {
                "../../Nexus/Assets/Scripts/Nexus/NexusGeneratedBinder.g.cs",
                "Nexus/Assets/Scripts/Nexus/NexusGeneratedBinder.g.cs",
            };
            for (int i = 0; i < candidates.Length; i++)
            {
                if (System.IO.File.Exists(candidates[i])) { file = candidates[i]; break; }
            }

            if (file == null)
            {
                // The binder is gitignored, so a fresh clone cannot inspect generated output.
                // Do not mark that as an unconditional pass: at least validate the generator
                // source that will produce the binder in the Unity editor.
                string generator = null;
                string[] generatorCandidates =
                {
                    "../../Nexus/Packages/com.nexus.core/Editor/CodeGen/NexusCodeGenerator.cs",
                    "Nexus/Packages/com.nexus.core/Editor/CodeGen/NexusCodeGenerator.cs",
                };
                for (int i = 0; i < generatorCandidates.Length; i++)
                {
                    if (System.IO.File.Exists(generatorCandidates[i])) { generator = generatorCandidates[i]; break; }
                }

                bool sourceGuardOk = false;
                string sourceDetail;
                if (generator == null)
                {
                    sourceDetail = "NexusCodeGenerator.cs not present; generated binder and source guard are both unavailable";
                }
                else
                {
                    try
                    {
                        string source = System.IO.File.ReadAllText(generator);
                        bool hasOptionalPath = source.Contains("OptionalInjectAttribute") && source.Contains("di.TryResolve<");
                        bool hasRequiredPath = source.Contains("di.Resolve<");
                        bool rejectsInvalidLink = source.Contains("Overwrite blocked") && source.Contains("<auto-generated>");
                        bool noDeadDispatcher = !source.Contains("CustomDispatcher = (");
                        sourceGuardOk = hasOptionalPath && hasRequiredPath && rejectsInvalidLink && noDeadDispatcher;
                        sourceDetail = $"generated binder absent; source guard: optional={hasOptionalPath}, required={hasRequiredPath}, linkGuard={rejectsInvalidLink}, noCustomDispatcher={noDeadDispatcher}";
                    }
                    catch (Exception ex)
                    {
                        sourceDetail = $"could not read NexusCodeGenerator.cs: {ex.Message}";
                    }
                }

                Check("AOT1. Binder_OptionalInject_TryResolveEmit", sourceGuardOk, sourceDetail);
                return;
            }
            ...
            {
                Check("AOT1. Binder_OptionalInject_TryResolveEmit", false,
                    $"could not read binder: {ex.Message}");
                return;
            }

            // The binder must mirror the runtime's [OptionalInject] semantics: optional
            ...
            bool noResolveForOptional = !text.Contains("di.Resolve<Nexus.Core.Services.INetworkEconomyValidator>")
                && !text.Contains("di.Resolve<Nexus.Core.Services.ILocalizationTableProvider>");
            ...
            bool ok = saveThrottlerInjects >= 2 && saveThrottlerClears >= 2
                ...
                && requiredStillResolves;

            Check("AOT1. Binder_OptionalInject_TryResolveEmit", ok,
                $"saveThrottlerInjects={saveThrottlerInjects} (>=2), saveThrottlerClears={saveThrottlerClears} (>=2), " +
                $"networkValidatorTryResolve={networkValidatorTryResolve}, tableProviderTryResolve={tableProviderTryResolve}, " +
                $"noResolveForOptional={noResolveForOptional}, requiredStillResolves={requiredStillResolves} " +
                $"— binder must stay in sync with NexusCodeGenerator + NexusDI");
        }
