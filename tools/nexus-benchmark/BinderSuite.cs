...
namespace NexusBench
{
    ...
    public static class BinderSuite
    {
        ...
        private static void RunBinderAotCompatibility()
        {
            ...
            if (file == null)
            {
                // Guard is machine-local by design: the binder is gitignored, so a fresh clone
                // has no file to check until the Unity editor generates it.
                Check("AOT1. Binder_OptionalInject_TryResolveEmit", true,
                    "NexusGeneratedBinder.g.cs not present (gitignored; generated by Unity editor) — skipped; drift is only checked where a binder exists");
                return;
            }
