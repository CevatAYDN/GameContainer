...
```powershell
dotnet run -c Release                            # benchmarks + recovery + 39 stress + fuzz + cross-context/thread + dogfood session + codegen
...
currently **86/86** (`Coverage` suite in the JSON doc lists every gap as a failed entry, so
...
| Suite | Covers |
| --- | --- |
| Benchmarks (`Program.cs`) | 1000-dispatch timing (<10ms), 1000-subscriber delivery, command pool reuse, `CommandPoolManager` Get/Return zero-alloc regression (<=128 B / 5000 ops), steady-state zero-GC (<=128 B / 5000 fires), 50k-dispatch stress (<200ms), hot-path ns/dispatch (<3000ns, <4000ns with subscriber — baseline ~360-440ns, ~7-8x CI headroom), FSM transition latency (<3000ns), HybridQueue thread-safe zero-GC, netcode rollback/replay zero-GC, ErrorCollection 4-thread stress (<500ms) |
...
| Service layer (`ServicesSuite.cs`) | 63 tests against the **real runtime** (real `Context`/`SignalBus`/`NexusRuntime`, functional Unity stubs — scene-graph hierarchy, `SceneManager` operation queue, `AudioSource` state, DI-injected prefs): Localization L1–L10 (RTL grapheme-safe reversal: basic Arabic, emoji surrogate pairs `A👋B🌍`→`🌍B👋A`, combining marks `a\u0301b`→`ba\u0301`, normalize+persist `NT_Language`, restart reload, fallback chain current→en→fallback→key, `RegisterLanguageTable`/`RegisterKey`, built-in TR table), Haptics H1–H3 (`NT_HapticsDisabled` gate, no-throw desktop path), Input I1–I4 (joystick clamp to unit magnitude + `PlayerMoveSignal` through the real bus, idle silence), Feedback F1–F4 (preset→haptic mapping incl. `ErrorFailure`→Heavy, preset audio handoff to `AudioService`, inverted-pitch guard), Audio A1–A8 (clamp+persist volumes, P0.3 `BgmStateMultiplier` NOT persisted + effective master×bgm×state math, muted zeroes, SFX pool 32-cap steal-oldest, muted no-alloc, `PlayBgm` dedup, inverted-pitch no-crash), FloatingText FT1–FT4 (field tracking, elapsed/expiry, min-duration clamp, empty/null ignored), Analytics AN1–AN2 (`NexusRuntime.Logger` routing, no-logger no-crash), SceneLoader S1–S6 (load/unload signal order, duplicate-load guard, missing-scene not-found, cancellation aborts + releases guard, `SetActiveScene` validates loaded), NexusTestContext T1–T6 (dispatch tracking, `AssertSignalNotDispatched` throws, clear, handler-less command throws `InvalidOperationException`, `CreateContext(configure, autoInitialize:true)` runs services via a real lifecycle's `OnConfigure`, child-context scoped teardown), UIManager U1–U7 (lifecycle callback order, concurrent-open dedup — duplicate rejected, `CloseTopScreen`, modal blocks lower layers + unblocks, snapshot sorted by open order, pooled reopen — same instance, dispose), Ads D1–D4 (cooldown honored via settable `Time.realtimeSinceStartup`, release-harness denial of mock rewards — no adapter ever grants currency, adapter delegation, negative-cooldown clamp + `OnImpressionRecorded`), IAP D5–D8 (catalog filter null/empty-id, no-adapter → `store_unavailable` at P0.2, adapter delegation, adapter-less purchase rejected), zero-GC G1–G4 (input update, pool-warmed `PlaySfx` ×2000, localization ×3000 lookups incl. TR, floating-text ×2000 — all ≤128 B) |
| Coverage (`CoverageReport.cs`) | Static report: every csproj Compile Include (incl. MSBuild `**` zero-dir globs) expanded against the runtime tree — compiled vs out-of-scope file list, captured as a `Coverage` suite in `--json` output (currently **86/86** compiled: every runtime file — `NexusDOTSBridge` is `#if UNITY_COLLECTIONS`-guarded (empty in the harness), `NexusDebugHUD` runs on no-op GUI stubs) |
| Codegen (`CodeGenSuite.cs`) | 2 tests against the **real generators**. **CG1** runs the editor-time `NexusCodeGenerator` (compiled with UnityEditor stubs, `GeneratorStubs.cs`): asserts the emitted binder contains the expected `RegisterConstructorFactory`/`RegisterInjector` lines (and skips value-type-param types; visibility + all-or-nothing gates; `[PostConstruct]` after generated injector), **compiles the emitted `NexusGeneratedBinder.g.cs` with the Roslyn shipped inside the .NET SDK** (no NuGet restore), invokes `NexusGeneratedBinder.Initialize()`, then resolves through the generated factories/injectors end to end: readonly ctor state, `[Construct]` selection, field injection, base-class chain walk, and `WithParameter` precedence over the generated factory. **CG2** drives the **shipped Roslyn Source Generator** (`NexusBinderGenerator`, same source compiled into the harness) through `CSharpGeneratorDriver` over a synthetic compilation (source-path fixtures in the current compilation + the harness assembly as a referenced game assembly), asserts the same gates, then compiles + boots its emitted binder end to end — including the metadata contract (injectors only for current-compilation types; referenced types get factories/dispatchers only, because Roslyn metadata hides private/internal members). Runs last — the binders register typed command dispatchers and injectors for every harness type, and the suite resets them in teardown |
| Registry (`RegistrySuite.cs`) | 37 tests against the **real runtime**: CommandRegistry registration validation (interface checks, both-interfaces rejection, async-as-sync rejection, mixed-mode, exclusive, duplicate priority, priority sort), composite validation (null/empty/65/duplicate), cross-context cache, the **fixed sync/async dispatchers** (generic-only commands actually dispatch — pre-fix they infinitely recursed / returned a stub) and **signal setter** (field/property paths + wrong-type guard), dispatcher cache stability, SubscriptionRegistry lifecycle (subscribe/unsubscribe/sweep, node-pool reuse identity, dispose), a **differential suite vs `SignalBus`** (DIFF1–9: identical registration sequences must produce identical exception type AND message), and zero-GC read paths (Z1–Z2: `HasAsyncSubscriptions`/read-copy + no-pending sweep, 0 B / 5000 ops) |
...
- **Bare contexts leaked every resolved `INexusService`**: `Context.Dispose` only
  disposed `INexusService` singletons when a `ContextBuilder` existed, so harness
  contexts that bound services directly through the container (never calling
  `Configure`) orphaned them. Soak caught it as `UnityEngine.Object.s_all +233`
  per iteration (window canvases, audio roots, BGM sources). Fixed in
  `Nexus/Packages/com.nexus.core/Runtime/Core/Context.cs`: the "remaining resolved
  singletons" pass now runs unconditionally — service lifecycle is owned by the
  Context per `NexusDI.Dispose`'s contract, so a missing builder must not skip it.
- **Soak's per-type object diff then exposed the rest** (17/iteration): the
  dogfood suite created a raw `ObjectPoolService` outside any context and never
  disposed it (`[Nexus_ObjectPool]` master root, `Pool_Enemy` root, 2 prewarmed
  `Enemy` instances), the child-context test leaked its 2 caller-owned
  `ContextData` ScriptableObjects, and the service suite never destroyed its 5
  test `AudioClip`s. All three now clean up in `finally`; soak runs 5/5 with
  `s_all` flat and the A5 baseline is now asserted absolute (0 → 32 sources).
...
SignalSubscriptions, CommandExecutor, RecoveryEngine, ContextLifecycleOrchestrator,
...
`Nexus/Packages/com.nexus.core/Runtime/` — all 86 runtime files.
...
- `GeneratorStubs.cs` — `UnityEditor.*` surface for the codegen file plus a real
  `AssemblyCatalog.RuntimeAssemblies()` predicate (mirrors the editor's) and a
  temp-directory `NexusEditorSettings`, so `CodeGenSuite` can run the real generator.

`CodeGenSuite` compiles the generated binder with the Roslyn that ships in the .NET SDK
(`$(MSBuildSDKsPath)\..\Roslyn\bincore`), so the "no NuGet dependencies" requirement
above stays true.
