...
| Coverage (`CoverageReport.cs`) | Static report: every csproj Compile Include (incl. MSBuild `**` zero-dir globs) expanded against the runtime tree — compiled vs out-of-scope file list, captured as a `Coverage` suite in `--json` output (currently 78/80 compiled; the 2 out-of-scope files are `NexusDebugHUD`, `NexusDOTSBridge` — editor/Jobs-platform stubs) |
