...
| Coverage (`CoverageReport.cs`) | Static report: every csproj Compile Include (incl. MSBuild `**` zero-dir globs) expanded against the runtime tree — compiled vs out-of-scope file list, captured as a `Coverage` suite in `--json` output (currently **80/80** compiled: every runtime file — `NexusDOTSBridge` is `#if UNITY_COLLECTIONS`-guarded (empty in the harness), `NexusDebugHUD` runs on no-op GUI stubs) |
