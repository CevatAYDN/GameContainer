...
currently **82/82** (`Coverage` suite in the JSON doc lists every gap as a failed entry, so
...
| Coverage (`CoverageReport.cs`) | Static report: every csproj Compile Include (incl. MSBuild `**` zero-dir globs) expanded against the runtime tree — compiled vs out-of-scope file list, captured as a `Coverage` suite in `--json` output (currently **82/82** compiled: every runtime file — `NexusDOTSBridge` is `#if UNITY_COLLECTIONS`-guarded (empty in the harness), `NexusDebugHUD` runs on no-op GUI stubs) |
