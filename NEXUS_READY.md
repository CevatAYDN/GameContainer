...
| 10 | **Dogfood örnek oyun** | Harness dogfood'u (GameSessionSuite) ✅; Unity örneği: tek kanonik örnek `Assets/Scripts/Game/Samples` — `NexusStarter.unity` sahnesine bağlı `GameLifecycle` + `GameView`, `NexusSetupWizard` ile üretilir (8 dosya: Lifecycle, View/Mediator, Service, Model, Command, Signal). Demo scaffolding (`Assets/Scripts/Demo/`) 2026-08-06'da silindi — mimaride tek örnek kaldı, örnek çoğaltması yok | ✅ Tamamlandı (2026-08-06: `Game/Samples` 8 dosya, sahne script GUID'leri %100 çözüldü — kırık referans yok; wizard şablonları mevcut örnekle birebir senkron, drift yok) |
