...
### Added
- **`Lifetime` enum + `CreateChildScope` + reverse-order disposal (competitive roadmap Faz 1)** — `NexusDI`/`IContextBuilder` gain `Lifetime.Singleton/Scoped/Transient` bind overloads (Singleton binds on the root container; Scoped = per-container, disposed with it; Transient = per-resolve, never owned), `NexusDI.CreateChildScope(configure)` for VContainer-style hierarchical scopes, and reverse-creation-order teardown in `Dispose`/`DisposeAsync`. Legacy `isSingleton` overloads map `true`→Scoped / `false`→Transient. Proven by `tools/nexus-benchmark/LifetimeScopeSuite.cs` (LT1–LT9).
- **Fluent binding chain + ref-based signal filters + AOT constructor factories (competitive roadmap Faz 1/2/4)** — `BindFluent<T>().To/AsSingleton/AsScoped/AsTransient/AsSingle/AsCached/AsImplementedInterfaces/AndSelf/WithParameter` on `NexusDI`/`IContextBuilder`/`ContextBuilder`; `ISignalFilter<T>` ref-based middleware on `SignalBus` (cancel/mutate, sync+async, zero-boxing — replaces the conditional 0-GC claim); `NexusCodeGenerator` now emits `NexusDI.RegisterConstructorFactory<T>` lambdas so IL2CPP constructs via direct `new T(...)` instead of `ConstructorInfo.Invoke` (readonly/immutable state works, strict/warn semantics preserved via `ResolveConstructorParameter<T>`). Proven by `tools/nexus-benchmark/NewFeatureSuite.cs` (FL1–FL7, SF1–SF5, CF1–CF3).
- **C# optional ctor params + value-type `WithParameter` (parity with Zenject `WithArguments`)** — ctor parameters are now satisfied by override → registered binding → declared C# default (e.g. `CommandPoolManager(NexusDI, int = 4)`); DI validation skips defaulted parameters. Harness pipeline is fully green again: the machine-level `dotnet` corruption (zeroed 10.0.11 set) was worked around with a user-space junction tree, harness stub gaps fixed, and the contradictory `NEXUS_DEBUG` define removed (real tracing allocates per event under it; E5/Stress-27 and the 0-GC audits assert the compiled-out contract). All suites pass: LifetimeScope LT1–LT9, NewFeature FL1–FL7/SF1–SF5/CF1–CF3, 50/50 Architecture Stress, GCAudit, ServiceGraph.
- **Doc-code consistency pass (roadmap Faz 0)** — `docs/ZENJECT_MIGRATION.md` no longer claims "constructor injection does not exist" (it does: `[Inject]`/`[Construct]`-marked or single public ctor); `STRANGE_COMPARISON.md` cross-boundary section now matches the existing `BindCrossBoundary`/`ResolveCrossBoundary` API; `CANONICAL-PATTERNS.md`/`AI_GAME_DEVELOPER_GUIDE.md` state the accurate mechanics of private-member injection (works via cached `FieldInfo.SetValue` fallback; public auto-property is the AOT-direct canonical); quickstarts (en+TR) and README examples teach the canonical public auto-property style.
