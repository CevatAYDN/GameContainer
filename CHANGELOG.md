...
- **ObservableList reentrant-drain allocation** — `DrainPendingChanges` replaced its per-pass `_pendingChanges.ToArray()` with a reusable growable buffer (amortized zero-allocation steady state).
